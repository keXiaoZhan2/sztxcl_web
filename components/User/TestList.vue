<template>
    <UiCard>
        <UiCardHeader class="flex justify-between">
            ...
            <span :class="item.answer_status ? 'text-green-600' : 'text-rose-600'">{{ item.answer_status ? '考试完成' : '未完成' }}</span>
        </UiCardHeader> 
        <UiCardBody>
            <small class="mr-3">题目总数：{{ item.testpaper.testpaper_question.length }}</small>
